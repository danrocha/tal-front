<template>
  <div :id="`location-${location.id}`" class="border-b">
    <div
      class="item flex flex-no-wrap items-start content-start justify-start w-full py-8 border-r-8 border-transparent hover:border-yellow-500"
      ...
      <div class="w-full ml-4">
        ...
        >
          <div v-if="location.office.sizeId" class="flex">
            <span
              class="uppercase text-xs py-1 px-2 mr-2 bg-white border border-gray-500rounded"
          ...
          update: (store, { data: { VOTE_MUTATION } }) => {
            ...
            this.scan(officeId, office => {
              if (alreadyVoted) {
                ...
                office.officeVotes.nodes = office.officeVotes.nodes.filter(
                  vote => {
